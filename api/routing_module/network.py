...
def load_network(osm_file_path=None):
    ...
    if osm_file_path is None:
        osm_file_path = os.path.join(os.path.dirname(__file__), 'data', 'new.osm')
...
def build_shape_data(gtfs_data):
    """
    Build shape lookup data for extracting trip paths.
    ...
    Returns:
        Dictionary containing:
            ...
            - shape_points: shape_id -> [[lat, lon], ...] sorted by pt_sequence
    """
    ...
    # shape_id -> sorted list of [lat, lon]
    shape_points = {}
    for shape_id, group in shapes.groupby('shape_id'):
        sorted_group = group.sort_values('shape_pt_sequence')
        shape_points[shape_id] = sorted_group[['shape_pt_lat', 'shape_pt_lon']].values.tolist()
    
    print(f"Built shape data:")
    print(f"  - {len(trip_to_shape)} trip-to-shape mappings")
    print(f"  - {len(shape_points)} unique shapes")
...
def create_lookup_dictionaries(gtfs_data):
    ...
    lookups = {
        ...
        'route_to_short_name_ar': routes.set_index('route_id')['route_short_name_ar'].to_dict() if 'route_short_name_ar' in routes.columns else {},
        ...
        'trip_to_headsign_ar': trips.set_index('trip_id')['trip_headsign_ar'].to_dict() if 'trip_headsign_ar' in trips.columns else {},
        ...
        'stop_to_name_ar': stops.set_index('stop_id')['stop_name_ar'].to_dict() if 'stop_name_ar' in stops.columns else {}
    }
...
def create_network(osm_file=None, gtfs_path=None, pathways_file=None, model_file=None, traffic_file=None, distance_file=None):
    ...
    # Build shape data
    print("\n[6/10] Building shape data...")
    shape_data = build_shape_data(gtfs_data)
    
    # Build  trip graph
    print("\n[7/10] Building  trip graph...")
    trip_graph = build_trip_graph(pathways)
    
    # Load cost model
    print("\n[8/10] Loading cost model...")
    cost_model = load_cost_model(model_file)
    
    # Load traffic data
    print("\n[9/10] Loading traffic data...")
    traffic_data = load_traffic_data(traffic_file)
    
    # Load distance data
    print("\n[10/10] Loading distance data...")
