...
class RoutingEngine:    
    def __init__(self, network_data):
        ...
        self.shape_data = network_data.get('shape_data', {})
    ...
    def get_trip_shape_path(self, trip_id, from_stop_id, to_stop_id):
        """
        Get the shape path (list of [lat, lon]) for a trip segment between two stops.
        
        Finds the nearest shape points to the from/to stop coordinates and
        returns the sub-path of the shape between them.
        
        Args:
            trip_id: GTFS trip ID
            from_stop_id: boarding stop ID
            to_stop_id: alighting stop ID
            
        Returns:
            List of [lat, lon] coordinates for the trip segment
        """
        trip_to_shape = self.shape_data.get('trip_to_shape', {})
        shape_points = self.shape_data.get('shape_points', {})
        ...
        from_coords = self.lookups['stop_to_coords'].get(from_stop_id)
        to_coords = self.lookups['stop_to_coords'].get(to_stop_id)
        if not from_coords or not to_coords:
            return []
        
        from_lat = from_coords['stop_lat']
        from_lon = from_coords['stop_lon']
        to_lat = to_coords['stop_lat']
        to_lon = to_coords['stop_lon']
        
        # Find nearest shape point index for each stop
        def nearest_index(lat, lon):
            best_i = 0
            best_dist = float('inf')
            for i, (pt_lat, pt_lon) in enumerate(points):
        ...
        start_idx = nearest_index(from_lat, from_lon)
        end_idx = nearest_index(to_lat, to_lon)
        
        if start_idx > end_idx:
            # Shape direction may be reversed relative to stop order
    ...
    def get_transport_time(self, trip_id, start_stop, end_stop):
        ...
        try:
            ...
            raise ValueError(f"Invalid numeric values for trip '{trip_id}': start='{start_val}', end='{end_val}'") from e

        return end_num - start_num

    def find_journeys_pareto(self, start_trips, goal_trips, max_transfers, restricted_modes=None):
        """
        ...
        for start_trip_id, data in start_trips.items():
            ...
            queue.append((start_trip_id, start_stop, start_sequence, path, c0, cost_details))
            ...
            if start_trip_id in goal_trips:
                ...
                if start_sequence < goal_seq:
                    delta_fare = self.get_fare(start_trip_id, start_stop, goal_stop, data['agency'])
                    delta_time = self.get_transport_time(start_trip_id, start_stop, goal_stop)
        ...
        while queue:
            (current_trip, current_board_stop, current_board_sequence, path, c, cost_details) = queue.popleft()
            ...
            for next_trip, pathway in self.trip_graph.get(current_trip, {}).items():
                ...
                if (pathway['start_agency_id'] in restricted_modes or 
                    pathway.get('end_agency_id', pathway['start_agency_id']) in restricted_modes):
                    continue
                ...
                new_cost_details = cost_details + [trip_cost_detail, transfer_cost_detail]
                ...
                queue.append((next_trip, pathway['end_stop_id'], pathway['end_stop_sequence'], new_path, c_new, new_cost_details))
                ...
                if next_trip in goal_trips:
                    ...
                    if transfer_in_seq < goal_seq:
                        last_leg_time = self.get_transport_time(next_trip, pathway['end_stop_id'], goal_stop)
                        last_leg_money = self.get_fare(next_trip, pathway['end_stop_id'], goal_stop, goal_trips[next_trip]['agency'])
    ...
    def rank_routing_results(self, routing_results, weights=None, top_n=5):
        ...
        for i, (trip_path, total_costs, cost_details) in enumerate(routing_results):
            ...
            norm_transfer = (total_costs[0] - ranges['transfer'][0]) / (ranges['transfer'][1] - ranges['transfer'][0]) if ranges['transfer'][1] > ranges['transfer'][0] else 0
            norm_fare = (total_costs[1] - ranges['fare'][0]) / (ranges['fare'][1] - ranges['fare'][0]) if ranges['fare'][1] > ranges['fare'][0] else 0
            norm_time = (total_costs[2] / 60 - ranges['time'][0]) / (ranges['time'][1] - ranges['time'][0]) if ranges['time'][1] > ranges['time'][0] else 0
            norm_walk = (total_costs[3] - ranges['walk'][0]) / (ranges['walk'][1] - ranges['walk'][0]) if ranges['walk'][1] > ranges['walk'][0] else 0
            
            # Calculate weighted score
            score = (weights['transfer'] * norm_transfer + 
            ...
            for detail in cost_details:
                if detail['type'] == 'trip':
                    ...
                    trip_leg = {
                        ...
                        "headsign": self.lookups['trip_to_headsign'].get(detail['trip_id'], "Unknown"),
                        "fare": round(detail['fare'], 2),
                        "duration_minutes": max(1, math.ceil(detail['time'] / 60)),
                        "from": {
                            "stop_id": detail['from_stop_id'],
                            "name": self.lookups['stop_to_name'].get(detail['from_stop_id'], "Unknown Stop"),
                            "coord": [from_coords.get('stop_lat', 0), from_coords.get('stop_lon', 0)]
                        },
                        "to": {
                            "stop_id": detail['to_stop_id'],
                            "name": self.lookups['stop_to_name'].get(detail['to_stop_id'], "Unknown Stop"),
                    ...
                    if pathway.get('walking_path_coords'):
                        try:
                            if isinstance(pathway['walking_path_coords'], str):
                                walking_coords = ast.literal_eval(pathway['walking_path_coords'])
                            else:
    ...
    def find_journeys(self, start_lat, start_lon, end_lat, end_lon, 
                     ...
                     weights=None, restricted_modes=None, top_k=5):
        """
